...
interface ITDT { // Standard ERC721 does not expose the `exists` method.
...
contract Turbomint {
    ...
    constructor(IERC20 _tbtcContract, ITDT _frtContract, ITDT _tdtContract) public {
    ...
    function cancelOrder(uint256 _tdtId) public {
			require(openOrders[_tdtId].requester != address(0x0), "No open order for the given TDT id");
        address originalRequester = openOrders[_tdtId].requester;
        require(msg.sender == originalRequester, "Only original TDT holder can cancel order.");
